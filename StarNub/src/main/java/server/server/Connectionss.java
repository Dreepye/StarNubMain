...
import server.connections.player.character.CharacterIP;
import server.connections.player.groups.GroupSync;
import server.connections.player.session.PendingPlayer;
import server.connections.player.session.Player;
import server.connections.player.session.Restrictions;
...
//        }
     ...
     * Starnub will create a {@link server.connections.player.session.PendingPlayer} which represents a pending
        ...
        StarNub.getPacketEventRouter().registerEventSubscription("StarNub", ClientConnectPacket.class, new PacketEventHandler() {
            ...
            * open sockets list. The open sockets list is to ensure no resources are leaked by attacks */
            ...
            /* Character check or create & db create/save */
                server.connections.player.character.Character character = cDb.getCharacters().getCharacterFromNameUUIDCombo(cliConPacket.getPlayerName(), cliConPacket.getUUID());
                if (character == null) {
...
//        });
     ...
     * pending player session data and create a new {@link server.connections.player.session.Player} that will then be
...
//        });
     ...
     * Recommended: For internal use with StarNub.
     * <p>
     * Uses: This is the core method for adding a player who is actively connect to the StarNub and Starbound starbounddata.packets.starbounddata.packets.server.
     * <p>
     ...
     * Recommended: For internal use with StarNub.
     * <p>
     * Uses: This is the core method for removing a player who is actively connect to the StarNub and Starbound starbounddata.packets.starbounddata.packets.server.
     * <p>
     * @param clientCtx ChannelHandlerContext representing the client side of the starbounddata.packets.connection to be removed
     */
     ...
     * Recommended: For internal use with StarNub.
     * <p>
     * Uses: This method will notify the starbounddata.packets.starbounddata.packets.server that a player is no longer connect in order to
