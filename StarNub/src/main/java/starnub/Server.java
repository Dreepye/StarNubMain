...
public enum Server {
    ...
    /**
     * References the starboundManager variable to the {@link starnub.server.starbound.OLDStarboundManager} Enum singleton.
     */
...
//        knownPackets.setPacketDebugging();
    ...
    protected void configStarboundConfiguration() {
        ...
        if (!configExist) {
            StarNub.getLogger().cInfoPrint("StarNub", "Starbound configuration has been generated, shutting starbounddata.packets.starbounddata.packets.starnub down.");
        ...
        if (!sbConfigBackup.exists()) {
            ...
            jsonObject.put("serverName", ((Map)StarNub.getConfiguration().getConfiguration().get("starbounddata.packets.starbounddata.packets.starnub info")).get("server_name"));
