 ...
 * @since 1.0
 */
public enum OLDStarboundManager {
    INSTANCE;
    ...
    public synchronized boolean startUp() {
        if(!starboundStatus.isOnline() && !starboundStatus.isStartingUp()) {
            ...
            StarboundManagement.startProcess();
            StarboundQueryTaskEvent.eventSend_Server_Query_Task_Starting();
            new Thread (new OLDStatusQuery("startup"),"StarNub - Starbound - Server Status Query").start();
            return true;
        } else {
            return false;
        }
    ...
    public synchronized boolean statusQuery() {
        ...
        if(starboundStatus.isOnline() && !starboundStatus.isStartingUp() && !starboundQueryTask.isRunning() && !starboundQueryTask.isStarting()) {
            StarboundQueryTaskEvent.eventSend_Server_Query_Task_Starting();
            new Thread (new OLDStatusQuery("responsiveness"),"StarNub - Starbound - Server Status Query").start();
            return true;
        } else {
            return false;
        }
    ...
    public synchronized boolean shutdown(boolean isRestarting) {
        ...
        if(starboundStatus.isOnline()) {
            ...
            StarboundManagement.stopProcess();
            while (StarboundManagement.isProcessRunning()) {
