 ...
 * <p>
 * When a player logs into the starbounddata.packets.starbounddata.packets.server, that is a character the {@link server.connectedentities.player.character.Character}
 * class represents this that character gets attached to a {@link server.connectedentities.player.session.Player} session, the Player
...
public class Account {
    ...
    public Account(Character player, String accountName, String accountPassword) {
        ...
        for (String groupName : StarNub.getServer().getConnections().getGroupSync().getGroups().keySet()) {
            Map<String, Object> group = (Map) StarNub.getServer().getConnections().getGroupSync().getGroups().get(groupName);
