...
import org.starnub.server.plugins.PluginPackage;
...
import org.starnub.server.server.datatypes.color.GameColors;
...
public enum MessageSender {
    ...
    /**
     * @return GameColors used in various areas of the game and chat
     */
     ...
     * {@link org.starnub.server.server.datatypes.color.GameColors} singleton enum.
     */
     ...
     *                         at {@link org.starnub.server.server.Connections} under method PlayerByAnyIdentifier.
    ...
    private boolean canWhisperPlayer(Player playerSender, Player playerReceiver){
        ...
        if (!con.hasPermission(playerSender, "starnubinternals.server.whisper", true)) {
            playerMessage("StarNub", playerSender, ChatReceiveChannel.UNIVERSE, "You do not have the permission \"starnub.server.whisper\" you cannot whisper.");
            return false;
        }
     ...
     * can be turned off, while failed commands will not be able to be disabled by server owners.
     * <p>
     *
     ...
     * Recommended: For Plugin Developers & Anyone else.
     * <p>
     * Uses: This is used to send a message to the server on a specified player channel. This method is useful
     ...
     * @param playerIdentifier Object which represents the player identifier can be a Player name, Player class,
     *                         Player IP, Player UUID, Client ID or ChannelHandlerContext (CTX) (Server or Client Side)
     * @param channel          String for starbound chat channel, acceptable is (planet, universe)
     * @param message          String the message to be sent to the player
     */
     ...
     * include the players name color or the servers default. Names are built using Nick Names, you can
     * opt to include the original Player Name.
     * <p>
     *
     * @param nameToBuild Object representing the name to be built
...
//                nameString = StarNub.getPluginManager().getPluginPackageClassNameString(string).getPLUGIN_NAME();
     ...
     * @param channel ChatChannel the type of reply (universe, planet, whisper, command)
     * @param message String the message to be sent to the player
     * @return ChatReceivePacket the chat packet to be sent to player
     */
