...
package org.starnub.server.eventsrouter.internaldebugging;
...
import org.starnub.server.server.packets.chat.ChatReceivePacket;
import org.starnub.server.server.packets.chat.ChatSendPacket;
import org.starnub.server.server.packets.global.PlayerWarpPacket;
import org.starnub.server.server.packets.server.ProtocolVersionPacket;
import org.starnub.server.server.packets.server.UniverseTimeUpdatePacket;
import org.starnub.server.server.packets.tile.DamageTileGroupPacket;
import org.starnub.server.server.packets.world.WorldStartPacket;
import org.starnub.server.server.packets.world.WorldStopPacket;
import org.starnub.server.server.packets.StarboundBufferReader;
import org.starnub.server.server.packets.connection.*;
...
* <p>
* This enum holds the set and get methods for known packets.
* <p>
...
public enum PacketDebugger {
    ...
    /**
     * This method sets the LinkedList and only works when the Starbound server is not online
     * due to the volatility of playing with this while needing them for player connections. This
     * method will also set up Packet Debugging by invoking eventsrouter located at
    ...
    public synchronized void setPacketDebugging() {
        ...
        Reflections reflections = new Reflections("org.starnub.server.packets");
        ...
        for (Object value : implementedPackets.values()) {
            ...
            for (Class c : allClasses) {
                ...
                try {
                    ...
                    StarNub.getLogger().cErrPrint("StarNub", "Could not load Packet Event Debugging Class.");
                }
                try {
                    if (((String) list.get(2)).equalsIgnoreCase("debug") && packetDebuggingClass != null) {
                        try {
                            ...
                            StarNub.getLogger().cErrPrint("StarNub", "Could not load Packet Event Debugging Method.");
    ...
    /**
     * Used in debugging {@link org.starnub.server.server.packets.chat.ChatReceivePacket}
     */
    ...
    /**
     * Used in debugging {@link org.starnub.server.server.packets.chat.ChatSendPacket}
     */
    ...
    /**
     * Used in debugging {@link org.starnub.server.server.packets.connection.ClientConnectPacket}
     */
    ...
    /**
     * Used in debugging {@link org.starnub.server.server.packets.connection.ClientDisconnectRequestPacket}
     */
    ...
    /**
     * Used in debugging {@link org.starnub.server.server.packets.connection.ConnectResponsePacket}
     */
    ...
    /**
     * Used in debugging {@link org.starnub.server.server.packets.tile.DamageTileGroupPacket}
     */
    ...
    /**
     * Used in debugging {@link org.starnub.server.server.packets.connection.HeartbeatPacket}
     */
        ...
        StarNub.getPacketEventRouter().registerEventSubscription("StarNub", HeartbeatPacket.class, new PacketEventHandler() {
            ...
            public Packet onEvent(Packet eventData) {
                ...
                StarNub.getLogger().cDebPrint("StarNub", "PACKET HeartbeatPacket Data: \n" +
                        "Packet ID: " + herBeaPacket.getPacketId() +
                        "\nPacket Payload: " + herBeaPacket.getCurrentStep());
                return eventData;
            }
        });
    }
    ...
    /**
     * Used in debugging {@link org.starnub.server.server.packets.server.ProtocolVersionPacket}
     */
    ...
    /**
     * Used in debugging {@link org.starnub.server.server.packets.connection.ServerDisconnectPacket}
     */
        ...
        StarNub.getPacketEventRouter().registerEventSubscription("StarNub", ServerDisconnectPacket.class, new PacketEventHandler() {
            ...
            public Packet onEvent(Packet eventData) {
                ...
                StarNub.getLogger().cDebPrint("StarNub", "PACKET ServerDisconnectPacket Data: \n" +
                        "Packet ID: " + serDisPacket.getPacketId() +
                        "\nPacket Payload: " + serDisPacket.getPayload());
                return eventData;
            }
        });
    }
    ...
    /**
     * Used in debugging {@link org.starnub.server.server.packets.server.UniverseTimeUpdatePacket}
     */
