...
import org.starnub.server.DateAndTimes;
import org.starnub.server.StarNub;
import org.starnub.server.cache.objects.TimeCache;
import org.starnub.server.cache.wrappers.PlayerUUIDCacheWrapper;
...
import org.starnub.server.connectedentities.player.groups.GroupSync;
import org.starnub.server.connectedentities.player.session.PendingPlayer;
...
import org.starnub.server.eventsrouter.events.ObjectEvent;
...
import org.starnub.server.server.chat.ChatFilter;
import org.starnub.server.server.datatypes.chat.ChatSendChannel;
import org.starnub.server.server.packets.Packet;
import org.starnub.server.server.packets.connection.ClientConnectPacket;
import org.starnub.server.server.packets.connection.ClientDisconnectRequestPacket;
import org.starnub.server.server.packets.connection.ConnectResponsePacket;
import org.starnub.server.server.packets.connection.ServerDisconnectPacket;
 ...
 * <p>
 * The data in this represents a pending player connection, connected player, whitelist and
...
//    private static Set<CachedData> cachedData;
     ...
     * and the server whitelist if they are null. Else they cannot be reset. After this we
...
//        }
     ...
     * Recommended: For internal use with StarNub.
     * <p>
     * Uses: This method adds an event handler to listen for player connection packets. It will then read the data
     * and check StarNubs internal configurations and databases/tables to determine how to handle the pending connection.
     * Starnub will create a {@link org.starnub.server.connectedentities.player.session.PendingPlayer} which represents a pending
    ...
    private void eventListenerPlayerConnectionAttempt() {
        ...
        StarNub.getPacketEventRouter().registerEventSubscription("StarNub", ClientConnectPacket.class, new PacketEventHandler() {
            ...
            * open sockets list. The open sockets list is to ensure no resources are leaked by attacks */
                ...
                Map confSI = (Map) conf.getConfiguration().get("server info");
            ...
            /* Character check or create & db create/save */
                Character character = cDb.getCharacters().getCharacterFromNameUUIDCombo(cliConPacket.getPlayerName(), cliConPacket.getUUID());
                if (character == null) {
            ...
            /* Account last login time and ID set & db create/save */
                ...
                //TODO ignore list This currently just sets it to not have a null list for the chat rooms
            ...
            /* TODO clean up player vip slots */
                ...
                if (currentPlayerCount >= playerLimit) {
                    if (hasPermission(playerSession, "starnubinternals.reserved.kick", true)) {
                        ...
                        if (currentPlayerCount > combinedCount) {
                            rejectionReason =
                                    "^#f5f5f5;This server is full and no more VIP slots are available.";
                            rejectClient = "server_full";
                        }
                    } else {
                        rejectionReason =
                                "^#f5f5f5;This server is full and you do not have permission to enter.";
                        rejectClient = "server_full";
                    }
                }
                ...
                StarNub.getLogger().cDebPrint("StarNub", "A Player is attempting to connect to the server on IP: " + connectingIp + ".");
...
//        StarNub.getPacketEventRouter().getPacketReactor().on(T(Packet.class), ev -> {
//
//        });
    }
     ...
     * Recommended: For internal use with StarNub.
     * <p>
     * Uses: This method will add an event handler to listen to the servers connection response to the player connection attempt.
     ...
     * pending player session data and create a new {@link org.starnub.server.connectedentities.player.session.Player} that will then be
...
//        StarNub.getPacketEventRouter().getPacketReactor().on(T(ConnectResponsePacket.class), ev -> {
        StarNub.getPacketEventRouter().registerEventSubscription("StarNub", ConnectResponsePacket.class, new PacketEventHandler() {
            ...
            /* Cast the event data (ev.data()) into a connection response packet */
            ...
            /* StarNub will spawn a thread to finish the client connection so that the packet can continue to the client
             * without delay. Delaying this packet further then the above will cause a race condition and a client
             * disconnect from the server, before the connection set up is complete */
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (pendingPlayer.getREJECT_CLIENT() != null) {
                            ...
                            if (rejectReason.equalsIgnoreCase("whitelist")) {
                                ...
                                cMsgSen.cWarnPrint("StarNub", "A Player tried connecting while the server is whitelisted on IP: " + pendingPlayer.getSESSION_IP() + ".");
                                ...
                                cMsgSen.cWarnPrint("StarNub", "A banned Player tried connecting to the server on IP: " + pendingPlayer.getSESSION_IP() + ".");
                                serverSideNotificationOfPlayerDisconnect(pendingPlayer.getSESSION_SERVER_CTX());
                                return;
                                ...
                                PlayerEvent.eventSend_Player_Connection_Failure_Character_Already_Online(pendingPlayer);
                                cMsgSen.cWarnPrint("StarNub", "A player tried to log into the server which is full.");
                                serverSideNotificationOfPlayerDisconnect(pendingPlayer.getSESSION_SERVER_CTX());
                                return;
                            } else if (rejectReason.equalsIgnoreCase("restarting")) {
                                PlayerEvent.eventSend_Player_Connection_Failure_Character_Already_Online(pendingPlayer);
                                cMsgSen.cWarnPrint("StarNub", "A player tried to log into the server while its preparing to restart.");
                                serverSideNotificationOfPlayerDisconnect(pendingPlayer.getSESSION_SERVER_CTX());
                                return;
                            }
                        }
                    ...
                    /* Sends an event on player connection success, adds the player to universe chat */
                        PlayerEvent.eventSend_Player_Connection_Success(playerSession);
                        //REPLACE with chat room subscriptions
...
//        });
     ...
     * on the OS side, this method will eventually be accompanied by more sure proof methods, such as a connection rate limiter
     ...
     * Recommended: For internal use with StarNub.
     * <p>
     * Uses: This is the core method for a player initializing a connection to StarNub and Starbound it will
     ...
     * a full session.
     * <p>
     * @param clientCtx ChannelHandlerContext representing the client side of the connection to be removed
     */
    private PendingPlayer removePendingConnections(ChannelHandlerContext clientCtx) {
        return pendingConnections.remove(clientCtx);
        //PLACE_HOLDER - Pending connection purge event
    }
     ...
     * Recommended: For internal use with StarNub.
     * <p>
     * Uses: This method well check all of the pending connections and remove any pending connection older then 5 minutes.
     * <p>
     */
    ...
    public void pendingConnectionPurge() {
        pendingConnections.values().stream().filter(pendingPlayer -> (System.currentTimeMillis() - pendingPlayer.getCONNECTION_PENDING_TIME()) >= 300000).forEach(pendingPlayer -> {
            StarNub.getLogger().cErrPrint("StarNub", "It appears that a pending player connection was never " +
     ...
     * Recommended: For internal use with StarNub.
     * <p>
     * Uses: This is the core method for adding a player who is actively connected to the StarNub and Starbound server.
     * <p>
     ...
     * Recommended: For internal use with StarNub.
     * <p>
     * Uses: This is the core method for removing a player who is actively connected to the StarNub and Starbound server.
     * <p>
     * @param clientCtx ChannelHandlerContext representing the client side of the connection to be removed
     */
     ...
     * Recommended: For internal use with StarNub.
     * <p>
     * Uses: This method will add an event handler to listen for client and server disconnect packets. StarNub uses this
     ...
     * Recommended: For internal use with StarNub.
     * <p>
     * Uses: This method will notify the server that a player is no longer connected in order to
     ...
     * Recommended: For Plugin Developers & Anyone else.
     * <p>
     * Uses: This method will force a connection to be closed for a players session.
     * <p>
     */
     ...
     * Recommended: For internal use with StarNub.
     * <p>
     * Uses: This method will send an event if a player has lost connection.
     * <p>
     ...
     * Recommended: For internal use with StarNub.
     * <p>
     * Uses: This method will clean up a players connection, by saving off database stats and ensuring
     * that both sides of the StarNub connection were closed.
     * <p>
     */
     ...
     * ensure that Starbound does self cleanup.
     * <p>
     * @param serverSideCTX ChannelHandlerContext that represents the server side of the client-server connection
     */
     ...
     * as the last seen Date. This ensures that the time is accurate in case of critical crash of the server tool
     * or improper restart.
     * <p>
     */
     ...
     * If not it will send a player lost connection message and remove them from the database.
     * <p>
     */
