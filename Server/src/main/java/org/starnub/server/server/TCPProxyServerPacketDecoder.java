...
import org.starnub.server.server.packets.misc.PassThroughPacket;
...
class TCPProxyServerPacketDecoder extends ReplayingDecoder<TCPProxyServerPacketDecoder.DecoderState> {
     ...
     * then this will spawn a connection through the proxy to the Starbound Server.
     *
    ...
    public void handlerAdded(ChannelHandlerContext ctx) throws Exception {
        if (CONNECTION_SIDE.equalsIgnoreCase("ServerSide")) {
            StarNubEventsInternals.eventSend_StarNub_Socket_Connection_Attempt_Server(ctx);
            StarNubEventsInternals.eventSend_StarNub_Socket_Connection_Success_Server(ctx);
            setPacketPool(ctx);
    ...
    private void setClientConnection(ChannelHandlerContext ctx){
        ...
        if (StarNub.getInternallyBannedIps().contains(connectingIp)) {
            StarNubEventsInternals.eventSend_StarNub_Socket_Connection_Failed_Blocked_IP_Client(ctx);
            ctx.close();
            return;
        } else {
            StarNubEventsInternals.eventSend_StarNub_Socket_Connection_Attempt_Client(ctx);
        }
        ...
        StarNubEventsInternals.eventSend_StarNub_Socket_Connection_Success_Client(ctx);
        setPacketPool(ctx);
    }
    ...
    private void setPacketPool(ChannelHandlerContext ctx) {
        ...
        Map<String, Object> packetCountMap = new YamlLoader().resourceYamlLoader("org/starnub/server/resources.yml");
    ...
    /**
     * This method will send a Player Disconnect Event {@link org.starnub.server.eventsrouter.events.PlayerEvent}
     *
    ...
    /**
     * This is set so we can collect information about connection issues.
     *
     * @param ctx   ChannelHandlerContext represents the context this handler is attached to
