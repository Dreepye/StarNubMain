...
import org.starnub.server.NamedThreadFactory;
import org.starnub.server.StarNub;
import org.starnub.server.eventsrouter.events.StarboundServerStatusEvent;
import org.starnub.server.eventsrouter.internaldebugging.PacketDebugger;
import org.starnub.server.server.chat.ServerChat;
//import org.starnub.server.packets.KnownPackets;
import org.starnub.server.server.packets.server.ProtocolVersionPacket;
import org.starnub.server.server.starbound.StarboundManager;
...
public enum Server {
    ...
    /**
     * References the starboundManager variable to the {@link org.starnub.server.server.starbound.StarboundManager} Enum singleton.
     */
...
////    }
    ...
    /**
     * References the serverChat variable to the {@link org.starnub.server.server.chat.ServerChat} Enum singleton.
     */
...
//        knownPackets.setPacketDebugging();
    ...
    protected void configStarboundConfiguration() {
        ...
        if (!configExist) {
            StarNub.getLogger().cInfoPrint("StarNub", "Starbound configuration has been generated, shutting server down.");
        ...
        if (!sbConfigBackup.exists()) {
            ...
            jsonObject.put("serverName", ((Map)StarNub.getConfiguration().getConfiguration().get("server info")).get("server_name"));
