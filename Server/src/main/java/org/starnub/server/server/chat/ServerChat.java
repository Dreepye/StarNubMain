...
package org.starnub.server.server.chat;
...
import org.starnub.server.server.Connections;
import org.starnub.server.server.Server;
import org.starnub.server.server.datatypes.chat.ChatReceiveChannel;
...
public enum ServerChat {
    ...
    /**
     * This method will intercept chat messages and insure they are routed correctly.
     * <p>
...
//        StarNub.getPacketEventRouter().getPacketReactor().on(T(ChatReceivePacket.class), this::serverChatProcessor);
    ...
    private ChatSendPacket chatMessage(long nowTime, Player messageSender, String chatMessage, ChatSendPacket chatPacket, boolean hasAccount) {
        ...
        Map confSC = (Map) StarNub.getConfiguration().getConfiguration().get("server chat");
        ...
        /* Can chat */
        if (!cConn.hasPermission(messageSender, "starnubinternals.server.chat", true)) {
            cMsgSen.playerMessage("StarNub", messageSender, "You do not have the permission \"starnubinternals.server.chat\" you can not use chat.");
            return chatPacket;
        }
        ...
        if (isMuted && !cConn.hasPermission(messageSender, "starnubinternals.bypass.mute", true)) {
            ...
            cMsgSen.playerMessage("StarNub", messageSender, "You are currently muted. Please visit " + ((String) ((Map) StarNub.getConfiguration().getConfiguration().get("server info")).get("server_url")) + " for further details.");
            return chatPacket;
        }

        /* Spam watch - Repeat messages */
        if ((boolean) confSC.get("global_message_block_repeat_messages") && !cConn.hasPermission(messageSender, "starnubinternals.chat.repeatmessage", true)) {
        ...
        if ((boolean) confSC.get("global_message_remove_continuous_letters") && !cConn.hasPermission(messageSender, "starnubinternals.chat.continuousletters", true)) {
            chatMessage = cChatF.removeMultipleCharacter(chatMessage);
        }

        /* Spam watch - Remove chat colors */
        if (!cConn.hasPermission(messageSender, "starnubinternals.chat.color", true)) {
            chatMessage = cChatF.removeColors(chatMessage);
        }

        /* Spam watch - Remove all upper case */
        if ((boolean) confSC.get("global_caps_limiter") && !cConn.hasPermission(messageSender, "starnubinternals.chat.nocapslimit", true)) {
    ...
    private void commandUse(long nowTime, Player messageSender, String chatMessage) {
        ...
        /* Can use commands */
        if (!cConn.hasPermission(messageSender, "starnubinternals.server.commands", true)) {
            StarNub.getMessageSender().playerMessage("StarNub", messageSender, "You do not have the permission \"starnubinternals.server.commands\" you can not use commands.");
            return;
        }
        ...
        if (isCommandBlocked && !cConn.hasPermission(messageSender, "starnubinternals.bypass.commandblock", true)) {
            ...
            cMsgSen.playerMessage("StarNub", messageSender, "You are currently unable to used commands. Please visit " + ((String) ((Map) StarNub.getConfiguration().getConfiguration().get("server info")).get("server_url")) + " for further details.");
            return;
        }
    ...
    private int personalChatRateSet(){
        return ((int) ((Map) StarNub.getConfiguration().getConfiguration().get("server chat")).get("global_message_speed"));
    }

    private int personalCommandRateSet(){
        return ((int) ((Map) StarNub.getConfiguration().getConfiguration().get("server chat")).get("global_command_speed"));
    }
