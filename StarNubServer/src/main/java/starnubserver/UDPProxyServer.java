...
/**
* This class will simply pass UDP traffic to starbounddata.packets.starbounddata.packets.starnubserver. This is
...
final class UDPProxyServer implements Runnable {
    ...
    public void run() {
		...
		/* Buffers for receive request and return the real starbounddata.packets.starbounddata.packets.starnubserver answer. */
		/* If we receive more then 1024 bytes from the starnubclient they will be discard. */
        byte[] request = new byte[1024];
        byte[] reply = new byte[4096];

        /* Loop until starbounddata.packets.starbounddata.packets.starnubserver is shutting down */
        while (true) {
            try {
				/* (1) receive data from starnubclient */
				 ...
				 * of bytes that we really received from starnubclient */
                byte[] real_request = new byte[from_client.getLength()];
                for (int i = 0; i < from_client.getLength(); i++) real_request[i] = request[i];

				/* (2) sending the received data to the starbounddata.packets.starbounddata.packets.starnubserver */
				...
				/* (3) reading the starbounddata.packets.starbounddata.packets.starnubserver answer */
				...
				/* (4) returning that answer to the starnubclient */
